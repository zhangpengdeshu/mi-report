class DeviceInfo {
  constructor() {
    const root = window;
    this._window = root || {};
    this.navigator = typeof root.navigator !== undefined ? root.navigator : {};
    this.infoMap = {
  ...
  // 在信息map和匹配库中进行匹配
  matchInfoMap(obj) {
    const u = this.navigator.userAgent || {}
    const match = this.getMatchMap(u)
    for (var s in this.infoMap) {
        for (var i = 0; i < this.infoMap[s].length; i++) {
            var value = this.infoMap[s][i]
            if (match[value]) {
              obj[s] = value
            }
        }
  ...
  getLanguage() {
    const self = this;
    self.language = (() => {
      const language = (self.navigator.browserLanguage || self.navigator.language);
  ...
  getBrowserInfo() {
    ...
    var _mime = function (option, value) {
        var mimeTypes = _this.navigator.mimeTypes
    ...
    var match = this.getMatchMap(u)

    var is360 = false
    if (this._window.chrome) {
        var chrome_vision = u.replace(/^.*Chrome\/([\d]+).*$/, '$1')
        if (chrome_vision > 36 && this._window.showModalDialog) {
            is360 = true
    ...
    if (is360) {
        if (_mime("type", "application/gameplugin")) {
            match['360SE'] = true
        } else if (this.navigator && typeof this.navigator['connection']['saveData'] == 'undefined') {
            match['360SE'] = true
        } else {
            match['360EE'] = true
        }
  ...
  // 获取地理位置
  getGeoPosition(obj) {
    this.navigator && this.navigator.geolocation && this.navigator.geolocation.getCurrentPosition((position) => {
      obj.position = {
...
export function getDeviceInfo() {
  ...
  const info = {
    ...
    screenHeight: device._window.screen.height,
    screenWidth: device._window.screen.width,
    language: device.getLanguage(),
    netWork: device.getNetwork(),
    orientation: device.getOrientationStatus(),
    browserInfo: device.getBrowserInfo(),
    userAgent: device.navigator.userAgent,
    date: device.getDate()
  };
  device.getGeoPosition(info);
  return info;
};
